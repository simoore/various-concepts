...
use crate::program::{
    Action, Arithmetic, ArithmeticOp, Assign, Block, Condition, ConditionOp, Direction, Expression,
    Identifier, IfThen, Program, SpecialIdentifier, Statement, Variable,
};
...
fn head_token(tokens: &[Token], error: CompilerError) -> Result<(&Token, &[Token]), CompilerError> {
    let token = tokens.first().ok_or(error.clone())?;
    Ok((token, &tokens[1..]))
}
...
fn get_identifier(tokens: &[Token]) -> Result<IdentifierReturn, CompilerError> {
    ...
    match token {
        ...
        Token::IdRest => Ok((
            Identifier::new_specialized(SpecialIdentifier::DailyRest),
            tokens,
        )),
        Token::IdEnergy => Ok((
            Identifier::new_specialized(SpecialIdentifier::Energy),
            tokens,
        )),
...
fn get_expression(tokens: &[Token]) -> Result<ExpressionReturn, CompilerError> {
    let token = tokens.first().ok_or(CompilerError::NoExpressionFound)?;
    match *token {
        Token::Lpar => {
            get_condition(tokens).map(|(cond, tkns)| (Box::new(cond) as Box<dyn Expression>, tkns))
        }
        Token::Add | Token::Sub => get_arithmetic(tokens)
            .map(|(arith, tkns)| (Box::new(arith) as Box<dyn Expression>, tkns)),
        _ => get_identifier(tokens).map(|(id, tkns)| (Box::new(id) as Box<dyn Expression>, tkns)),
    }
}
...
fn get_assign<'a>(tokens: &'a [Token], key: &str) -> Result<StatementReturn<'a>, CompilerError> {
    let var = Variable::new(key.to_owned());
...
fn get_ifthen(tokens: &[Token]) -> Result<StatementReturn, CompilerError> {
    ...
    if *else_or_end == Token::End {
        return Ok((
            Box::new(IfThen::new(condition, then_statement, None)),
            tokens,
        ));
    ...
    Ok((
        Box::new(IfThen::new(condition, then_statement, Some(else_statement))),
        tokens,
    ))
}
...
fn get_block(tokens: &[Token]) -> Result<StatementReturn, CompilerError> {
    ...
    loop {
        let token = loop_tokens.first().ok_or(CompilerError::NoEndBlockFound)?;
        if *token == Token::End {
...
pub fn get_program(text: &str) -> Result<Program, CompilerOrLexerError> {
    let tokens = tokenize(text).map_err(CompilerOrLexerError::L)?;
    let tokens = consume_token(Token::Sim, &tokens, CompilerError::NoSimKeyword)
        .map_err(CompilerOrLexerError::C)?;
    let (statement, tokens) = get_statement(tokens).map_err(CompilerOrLexerError::C)?;
...
mod tests {
    ...
    fn test_consume_token() {
        ...
        let tokens =
            consume_token(Token::DirE, &tokens, CompilerError::NotADirectionToken).unwrap();

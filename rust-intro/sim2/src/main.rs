...
impl SimUi {
    ...
    fn new(app: &gtk::Application) -> SimUi {
        ...
        SimUi { drawing_area, scrolled_window, sep, boxx, window, prey_entry, pred_entry }
    }
}
...
fn play(simui: Rc<RefCell<SimUi>>, sim: Rc<RefCell<Simulation>>) {
    ...
    glib::timeout_add_local(Duration::from_millis(1000), move || {
        ...
        if cont { glib::ControlFlow::Continue } else { glib::ControlFlow::Break }
    });
}
...
fn show_message(simui: &SimUi, msg: &str) {
    gtk::AlertDialog::builder().message(msg).build().show(Some(&simui.window));
}
...
fn config(simui: Rc<RefCell<SimUi>>, sim: Rc<RefCell<Simulation>>) {
    ...
    let pred_num = match simui.borrow().pred_entry.text().parse::<i32>() {
        Ok(n) => n,
        Err(_) => { show_message(&simui.borrow(), "Error parsing pred num"); return; }
    };

    let prey_num = match simui.borrow().prey_entry.text().parse::<i32>() {
        Ok(n) => n,
        Err(_) => { show_message(&simui.borrow(), "Error parseing prey num"); return; }
    };
...
fn draw(sim: Rc<RefCell<Simulation>>, _area: &gtk::DrawingArea, cairo: &cairo::Context, _width: i32, _height: i32) {
    ...
    for x in 0..100 {
        for y in 0..100 {
            match sim.borrow().grid().loc(x, y).status() {
                ...
                LocationStatus::NothingHere => cairo.set_source_rgb(1.0, 1.0, 1.0),
            };
...
fn build_ui(sim: Rc<RefCell<Simulation>>, app: &gtk::Application) {
    ...
    simui.borrow().drawing_area.set_draw_func(move |a, c, w, h| draw(sim.clone(), a, c, w, h));
...
fn main() -> glib::ExitCode {
    ...
    // simulation.
    let sim = Rc::new(RefCell::new(Simulation::new(usize::try_from(XGRID).unwrap(), usize::try_from(YGRID).unwrap())));
